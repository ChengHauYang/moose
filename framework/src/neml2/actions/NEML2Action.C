    ...
    _skip_input_variables(getParam<std::vector<std::string>>("skip_input_variables"))
{
...
#ifdef NEML2_ENABLED
  // Route NEML2's log output to MOOSE's console before any model is built/introspected.
  NEML2Utils::redirectLogsToConsole(_console);
  ...
  // Python extensions); cpp-aoti reads the compiled-artifact folder 'input'. 'cli_args' is
  // currently not forwarded (NEML2 v3's load takes no extra parse arguments).
  ...
  if (db.size() != 1 && db.size() != devices.size())
    paramError("device_batch", "'device_batch' must have length 1 or the same length as 'device'.");
...
NEML2Action::act()
{
  ...
  if (_current_task == "add_user_object")
  {
    ...
    // NEML2 target is the fully-qualified parameter name; the MOOSE source is the user-written
    // name.
    std::vector<UserObjectName> param_gatherers;
    for (const auto & param : _params)
      param_gatherers.push_back(addGatherer(
          param.moose_name, param.neml2_name, param.moose_type, param.moose_tensor_type, "param"));
...
NEML2Action::setupInputMappings(const NEML2ModelHandle & model)
{
  ...
  const auto & explicit_inputs = getParam<std::vector<std::string>>("inputs");
  ...
  for (const auto & vname : model.input_names())
  {
    ...
    // An order-0 input that is ALSO a model output is an in-place-updated unknown (e.g. an
    // ImplicitUpdate initial guess that has no old counterpart). The default gatherer would read
    ...
    // Newton solve from converging. Unless the user maps it explicitly, cold-start it at zero: skip
    // the gatherer here and let the executor seed a base-shaped zero (broadcast over the batch).
    if (order == 0 && contains(model.output_names(), vname) && !contains(explicit_inputs, vname))
    {
      mooseInfo("NEML2 input '",
                vname,
                "' is also a model output (an in-place-updated unknown), so it is cold-started at "
                "zero on every evaluation instead of gathered from its stale output material. To "
                "supply an explicit initial guess instead, map it via 'inputs'/'moose_inputs'.");
      continue;
    }
    ...
    if (mtype.empty())
      mooseError(
          "NEML2 input variable ", vname, " has a base shape not yet mapped to a MOOSE type.");

    _inputs.push_back(
        {base, inferMOOSEIOType(base, base_shape.empty()), mtype, static_cast<std::size_t>(order)});
  }
...
NEML2Action::setupOutputMappings(const NEML2ModelHandle & model)
{
  ...
  for (const auto & oname : model.output_names())
  {
    ...
    if (mtype.empty())
      mooseError(
          "NEML2 output variable ", oname, " has a base shape not yet mapped to a MOOSE type.");
...
NEML2Action::checkStatefulConsistency(const NEML2ModelHandle & model) const
{
  ...
  // current input or produce it as an output. MOOSE sources the old value from the material
  // property declared either by another material (when 'var' is also a current model input, e.g.
  // strain provided by the mechanics) or by this model's output retriever (when 'var' is a model
  // output). If the model does neither, nothing declares the property whose old value is requested,
  // and MOOSE segfaults during stateful material property initialization. The common offender is a
  // state variable pruned from the top-level outputs because a downstream model consumes it (e.g.
  // GeneralElasticity reads 'orientation'); the fix is to add it to the model's
  // 'additional_outputs'.
  for (const auto & input : _inputs)
    if (input.history_order > 0 && input.moose_type == NEML2Utils::MOOSEIOType::MATERIAL &&
        !contains(model.input_names(), input.name) && !contains(model.output_names(), input.name))
      mooseError(
          "The NEML2 model takes the old value of '",
          input.name,
          "' (",
          lagName(input.name, input.history_order),
