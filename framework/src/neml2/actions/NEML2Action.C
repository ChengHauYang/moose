...
NEML2Action::validParams()
{
  ...
  params.addParam<bool>(
      "interface_only",
      false,
      "If true, only create the interface material on the boundaries listed in 'interface' and skip "
...
NEML2Action::isInterfaceMaterialInput(const std::string & moose_name) const
{
  ...
  const auto parameterNamesContain = [](const auto & params,
  ...
  const auto parameterNameMatches = [](const auto & params,
                                       const std::string & param_name,
                                       const std::string & moose_name)
  {
