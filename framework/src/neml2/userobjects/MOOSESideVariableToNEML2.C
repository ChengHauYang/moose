...
registerMooseObject("MooseApp", MOOSESideOldVariableToNEML2);

template <unsigned int state>
InputParameters
MOOSESideVariableToNEML2Templ<state>::validParams()
{
  auto params = MOOSEToNEML2BatchedSide::validParams();
  params.addClassDescription("Gather a MOOSE side variable for insertion into the specified "
                             "input or model parameter of a NEML2 model.");
...
MOOSESideVariableToNEML2Templ<0>::MOOSESideVariableToNEML2Templ(const InputParameters & params)
  : MOOSEToNEML2BatchedSide(params)
#ifdef NEML2_ENABLED
    ,
