...
NEML2SideBatchIndexGenerator::validParams()
{
  ...
  params.addClassDescription(
      "Generates the side to batch index map for MOOSEToNEML2 gatherers, "
...
NEML2SideBatchIndexGenerator::NEML2SideBatchIndexGenerator(const InputParameters & params)
  : SideUserObject(params), _outdated(true)
{
}
...
NEML2SideBatchIndexGenerator::execute()
{
  ...
  key.boundary = _current_boundary_id;

  _side_to_batch_index[key] = _batch_index;
  _side_to_nqp[key] = _qrule->n_points();
...
NEML2SideBatchIndexGenerator::threadJoin(const UserObject & uo)
{
  ...
  for (const auto & [key, nqp] : m2n._side_to_nqp)
    _side_to_nqp[key] = nqp;
...
std::size_t
NEML2SideBatchIndexGenerator::getBatchIndex(dof_id_type elem_id,
                                            unsigned int side,
                                            BoundaryID boundary) const
{
  SideKey key{elem_id, side, boundary};

  if (_side_to_batch_index_cache.first.elem_id == key.elem_id &&
      _side_to_batch_index_cache.first.side == key.side &&
      _side_to_batch_index_cache.first.boundary == key.boundary)
