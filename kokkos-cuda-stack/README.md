...
against PyTorch and installs as a Python package. Layout choice: PyTorch +
NEML2 live in a dedicated **venv** at
`kokkos-cuda-stack/neml2-venv/` (gitignored). PETSc / libmesh / WASP /
OpenMPI / MOOSE stay in the from-scratch stack. `build_moose.sh` and
`build_neml2.sh` prepend `neml2-venv/bin` to `PATH` *after* `$PREFIX/bin`,
so `mpicxx` remains ours (CUDA-aware OpenMPI) while `python3` comes from
the venv (has NEML2 installed).

Why a venv, not the main `moose` conda env: `moose` is on Python 3.14, but
PyTorch's CUDA-12.4 wheels (the highest CUDA driver 550.x on this box
supports) stop at Python 3.13. `build_neml2.sh` creates the venv on first
run from miniforge base's `python3` (Python 3.12 -- newest cu124-compatible
Python already installed on this box, no `conda create`, no `apt install`).
No setup command needed from the user; `build_neml2.sh` handles it.
