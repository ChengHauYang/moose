nx = 64
[Problem]
  solve = false
[]

[Mesh]
  [boundary_mesh]
    type = FileMeshGenerator
    file = 'star.msh'
  []

  [shift_boundary_mesh]
    type = TransformGenerator
    transform = TRANSLATE
    vector_value = '2.0 2.0 0.0' # translation in x, y, z directions
    input = boundary_mesh
    save_with_name = 'shift_boundary_mesh'
  []

  [boundary_mesh2]
    type = FileMeshGenerator
    file = 'bunny_2D.msh'
  []
  [shift_boundary_mesh2]
    type = TransformGenerator
    transform = TRANSLATE
    vector_value = '0.5 2.8 0.0' # translation in x, y, z directions
    input = boundary_mesh2
    save_with_name = 'shift_boundary_mesh2'
  []

  [gen]
    type = CartesianMeshGenerator
    dim = 2
    dx = '4'
    dy = '4'
    ix = '${nx}'
    iy = '${nx}'
    subdomain_id = '1'
  []

  add_subdomain_ids = 2 # outside block
  add_sideset_names = 'SBMinterface'
  final_generator = 'gen'
[]

[UserObjects]
  [TreeBuilder]
    type = SBMSurfaceMeshBuilder
    surface_mesh = shift_boundary_mesh
  []
  [TreeBuilder2]
    type = SBMSurfaceMeshBuilder
    surface_mesh = shift_boundary_mesh2
  []
  [distance_uo]
    type = ShortestDistanceToSurface
    surfaces = 'dist_star dist_bunny circle1 flower1 ellipse_rot'
  []
[]

[Variables]
  [u]
    initial_condition = 1
    block = 1
  []
[]

[Functions]
  [dist_star]
    type = UnsignedDistanceToSurfaceMesh
    builder = TreeBuilder
  []

  [dist_bunny]
    type = UnsignedDistanceToSurfaceMesh
    builder = TreeBuilder2
  []

  [circle1]
    type = ParsedFunction
    expression = "sqrt((x-1.0)^2 + (y-1.0)^2) - 0.5"
  []
  [ellipse_rot]
    type = ParsedFunction
    symbol_names = 'a b theta cx cy'
    symbol_values = '0.8 0.4 0.785398 3.0 1.0'
    expression = "
      (
        (
          ((x-cx)*cos(theta) + (y-cy)*sin(theta)) / a
        )^2
        +
        (
          (-(x-cx)*sin(theta) + (y-cy)*cos(theta)) / b
        )^2
      )^(0.5) - 1
    "
  []
  [flower1]
    type = ParsedFunction
    expression = "
      ( abs(x-3.0)^4 + abs(y-3.0)^4 )^(0.25 ) - 0.6
    "
  []
[]

[AuxVariables]
  [distance]
    order = CONSTANT
    family = MONOMIAL
  []
[]

[AuxKernels]
  [dist]
    type = DistanceAux
    distance_uo = distance_uo
    variable = distance
    execute_on = 'INITIAL timestep_begin'
  []
[]

[Adaptivity]
  marker = refine_based_on_aux
  max_h_level = 5
  steps = 3
  [Markers]
    [refine_based_on_aux]
      type = ValueThresholdMarker
      variable = distance
      refine = 0.05
      coarsen = 0.5
      invert = true
    []
  []
[]

[Executioner]
  type = Steady
[]

[Outputs]
  exodus = true
[]
