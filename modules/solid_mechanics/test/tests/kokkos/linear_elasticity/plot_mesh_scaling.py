#!/usr/bin/env python3
"""Standalone plotter for mesh_scaling.csv (or any CSV matching the same
schema produced by benchmark_mesh_scaling.py).

Independent of the benchmark script — reads the CSV, computes mean +/- s.d.
per (device, mesh_size), and writes a two-panel PNG:
  top    : wall time vs mesh size (log-log), CPU and GPU with error bars
  bottom : CPU/GPU speedup ratio annotated per point

Also prints the table to stdout so you can eyeball the numbers without
opening the PNG.

Usage:
  ./plot_mesh_scaling.py                         # reads ./mesh_scaling.csv
  ./plot_mesh_scaling.py --csv path/to.csv       # explicit CSV
  ./plot_mesh_scaling.py --plot path/to.png      # explicit PNG output
  ./plot_mesh_scaling.py --title 'my title'
"""
import argparse
import csv
import statistics
...
def parse_args():
    ...
    parser.add_argument(
        "--csv",
        type=Path,
        default=SCRIPT_DIR / "mesh_scaling.csv",
        help="Input CSV (default: ./mesh_scaling.csv)",
    )
    parser.add_argument(
        "--plot",
        type=Path,
...
def read_grouped(csv_path):
    """Return {(device, mesh_size): [wall_time, ...], ...}.

    Accepts either the mesh_scaling.csv columns (device, mesh_size, elements,
    ..., wall_time_seconds) or a subset. Missing columns are ignored.
    """
    grouped = {}
...
def summarise(grouped):
    """Yield (device, mesh_size, n, mean, stdev) rows in sorted order."""
